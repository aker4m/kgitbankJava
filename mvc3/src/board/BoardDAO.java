...
public class BoardDAO {
	...
	public int writeBoard(BoardDTO bean) throws SQLException{
		String sql = null;
		if(bean.getNum()==0){
			sql="update mvcboard set re_step = re_step + 1"; 
		}else{
			sql="update mvcboard set re_step = re_step + 1 where re_step >"+bean.getRe_step();
		...
		try{
			...
			ps.executeUpdate();
			sql = "insert into mvcboard values(board_seq.nextval,?,?,?,?,sysdate,default,?,?,?,?)";
			ps = con.prepareStatement(sql);
	...
	public int writeBoard(MultipartRequest mr, String ip) throws SQLException{
		...
		if(Integer.parseInt(mr.getParameter("num")) == 0){
			...
			sql="update mvcboard set re_step = re_step + 1 where re_step >"+Integer.parseInt(mr.getParameter("re_step"));
	...
	public List<BoardDTO> listBoard(int start, int end) throws SQLException{
		String sql = "select * from (select rownum rn, A.* from (select * from mvcboard order by re_step asc)A) where rn between ? and ?";
		try{
			con = ds.getConnection();
			ps = con.prepareStatement(sql);
			ps.setInt(1, start);
	...
	public BoardDTO getBoard(int num, String mode) throws SQLException{
		...
		try{
			...
			ps.setInt(1, num);
			rs = ps.executeQuery();
			List<BoardDTO> list = this.makeList(rs);
			return list.get(0);
		}finally{
			if(rs!=null) rs.close();
			if(ps!=null) ps.close();
	...
	public boolean isPassword(int num, String passwd) throws SQLException{
		String sql = "select passwd from mvcboard where num=?";
		try{
			con = ds.getConnection();
			ps = con.prepareStatement(sql);
			ps.setInt(1, num);
	...
	public int updateBoard(BoardDTO bean) throws SQLException{
		String sql = "update mvcboard set writer=?, subject=?, email=?, content=? where num=?";
		try{
			con = ds.getConnection();
			ps = con.prepareStatement(sql);
			ps.setString(1, bean.getWriter());
