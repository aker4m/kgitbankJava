...
<%
	...
	BoardDBBean bean = bdBean.getBoard(Integer.parseInt(num), "content");
%>
...
<% 			}%>
			<tr>
				<td colspan="4" align="center" bgcolor="yellow">
					<input type="button" value="답글쓰기" onclick="window.location='writeForm.jsp?num=<%=bean.getNum()%>&re_step=<%=bean.getRe_step()%>&re_level=<%=bean.getRe_level()%>'">
					<input type="button" value="글수정" onclick="window.location='updateForm.jsp?num=<%=bean.getNum()%>'">
					<input type="button" value="글삭제" onclick="window.location='deleteForm.jsp?num=<%=bean.getNum()%>'">
